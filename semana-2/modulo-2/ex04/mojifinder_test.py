...
def test_name_endpoint() -> None:
    """Test the /search endpoint returns correct characters and names."""
    # Search for heart emoji characters
    response = client.get("/names?q=heart")
    ...
    assert "LATIN SMALL LETTER H" in data[0]['name']
    assert "LATIN SMALL LETTER E" in data[1]['name']
    assert "LATIN SMALL LETTER A" in data[2]['name']
    assert "LATIN SMALL LETTER R" in data[3]['name']
    assert "LATIN SMALL LETTER T" in data[4]['name']
