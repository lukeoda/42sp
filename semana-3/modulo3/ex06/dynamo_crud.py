...
from decode import ler_salvar_conteudo
...
def load_user(filename: str, table):
    try:
        ...
        with open(filename) as f:
            ...
            for row in reader:
                uuid_img = ler_salvar_conteudo(row['document'])
                table.put_item(
                    Item={
                    'Id': row["id"],
                    'Nome': row["name"],
                    'Document_key': str(uuid_img)
                    }
                )
...
if __name__ == "__main__":
    '''
    Uso: 
    python3 dynamo_crud.py load "./users/users_with_documents.csv"
    python3 dynamo_crud.py update "u448" "LUCAS ODA" 
    python3 dynamo_crud.py retrieve u448
    python3 dynamo_crud.py delete u448
    '''
    main()
